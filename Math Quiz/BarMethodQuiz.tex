...
\begin{center}
  ...
  {\Large \textbf{Part-Whole \& Comparison Models}}
\end{center}
\vspace{1em}

\section*{Part 1: Visualizing Addition}

% ================= EXAMPLE SECTION PART 1 =================
...
{\Large \textbf{Practice Questions: Addition \& Subtraction}}\par\vspace{0.4em}
...
%% ================= EXAMPLE SECTION PART 2 =================

{\Large \textbf{Example: Comparing Quantities}}

\vspace{0.3em}

\noindent Sam has \textbf{15 stickers}. He has \textbf{4 more stickers} than Leo. How many stickers does Leo have?

\vspace{0.8em}

\begin{description}[leftmargin=2em]

\item[\stepnum{1}{s21}] \textbf{Draw parallel bars for each subject}\\[0.2em]

Align the start of both bars to easily visualize the difference:

\vspace{0.6em}
\begin{tikzpicture}[>=Stealth, baseline={(0,0.5)}]

% Sam's Bar
\node[anchor=east] at (-0.2, 1.2) {\small Sam:};
\draw[fill=redbar, draw=black, line width=0.5pt] (0, 0.9) rectangle (4.5, 1.4);
\draw[decorate, decoration={brace, amplitude=4pt}, line width=0.5pt] (0, 1.5) -- (4.5, 1.5);
\node[anchor=south] at (2.25, 1.6) {\small 15 stickers};

% Leo's Bar
\node[anchor=east] at (-0.2, 0.3) {\small Leo:};
\draw[fill=bluebar, draw=black, line width=0.5pt] (0, 0.0) rectangle (3.3, 0.5);
\draw[decorate, decoration={brace, amplitude=4pt, mirror}, line width=0.5pt] (0, -0.1) -- (3.3, -0.1);
\node[anchor=north] at (1.65, -0.2) {\small ?};

% Difference line and bracket
\draw[dashed, line width=0.5pt] (3.3, -0.1) -- (3.3, 1.4);
\draw[decorate, decoration={brace, amplitude=4pt}, line width=0.5pt] (3.3, 1.5) -- (4.5, 1.5);
\node[anchor=south] at (3.9, 1.6) {\small 4 more};
\end{tikzpicture}

\vspace{0.6em}

\item[\stepnum{2}{s22}] \textbf{Subtract the difference to find the smaller bar}\\[0.4em]
\(\displaystyle 15 - 4 = \mathbf{11}\)

\vspace{0.2em}

Leo has \textbf{11 stickers}.

\end{description}

\begin{tikzpicture}[remember picture, overlay]
  \draw[dotted, thick, textlight!60] (s21.south) -- (s22.north);
\end{tikzpicture}
...
{\Large \textbf{Practice Questions: Comparison Models}}
\vspace{1em}
...
\begin{enumerate}[label=\textbf{\arabic*.}, start=6, itemsep=2.5em, leftmargin=2em] 
  ...
  \vspace{0.6em}
  $4 \text{ units} + 40 = 360 \implies 4 \text{ units} = \underline{\hspace{1.5cm}} \implies 1 \text{ unit} = \underline{\hspace{1.5cm}}$ \\ [0.2em]
  ...
  \vspace{0.6em}
  2 units = 36 $\implies$ 1 unit = $\underline{\hspace{1.2cm}}$ \\ [0.2em]
...
% ================= EXAMPLE SECTION PART 3 =================
{\Large \textbf{Example: Scaling with Equal Units}}
\vspace{0.3em}

\noindent A book costs \textbf{4 times as much} as a notebook. If the book costs \textbf{\$24}, how much does the notebook cost?
\vspace{0.8em}

\begin{description}[leftmargin=2em]

  \item[\stepnum{1}{s31}] \textbf{Draw unit blocks for each item}\\[0.2em]
  Represent the notebook as 1 unit and the book as 4 equal units:
  \\[0.6em]
  \begin{tikzpicture}[>=Stealth, baseline={(0,0.5)}]
    % Book Bar
    \node[anchor=east] at (-0.2, 0.9) {\small Book:};
    \draw[fill=redbar, draw=black, line width=0.5pt] (0, 0.65) rectangle (4.0, 1.15);
    \foreach \x in {1.0, 2.0, 3.0} { \draw[line width=0.5pt] (\x, 0.65) -- (\x, 1.15); }
    \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.5pt] (0, 1.25) -- (4.0, 1.25);
    \node[anchor=south] at (2.0, 1.35) {\small 4 units = \$24};

    % Notebook Bar
    \node[anchor=east] at (-0.2, 0.0) {\small Notebook:};
    \draw[fill=bluebar, draw=black, line width=0.5pt] (0, -0.2) rectangle (1.0, 0.3);
    \draw[decorate, decoration={brace, amplitude=4pt, mirror}, line width=0.5pt] (0, -0.3) -- (1.0, -0.3);
    \node[anchor=north] at (0.5, -0.4) {\small 1 unit = ?};
  \end{tikzpicture}
  \vspace{0.6em}

  \item[\stepnum{2}{s32}] \textbf{Divide total value by number of units}\\[0.4em]
  \[
    4 \text{ units} = \$24 \implies 1 \text{ unit} = 24 \div 4 = \mathbf{6}
  \]
  \vspace{0.2em}
  The notebook costs \textbf{\$6}.

\end{description}

\begin{tikzpicture}[remember picture, overlay]
  \draw[dotted, thick, textlight!60] (s31.south) -- (s32.north);
\end{tikzpicture}
...
{\Large \textbf{Practice Questions: Scaling \& Repeated Identity}}
\vspace{1em}
...
\begin{enumerate}[label=\textbf{\arabic*.}, start=11, itemsep=2.5em, leftmargin=2em]
  ...
  \vspace{0.6em}
  4 units = \$640 $\implies$ 1 unit = $\$640 \div 4 = \underline{\hspace{1.5cm}}$ \\ [0.2em]
  ...
  \vspace{0.6em}
  Remaining total = $\$180 - \$30 = \underline{\hspace{1.5cm}}$ \\ [0.2em]
  3 units = $\underline{\hspace{1.5cm}} \implies 1 \text{ unit} = \underline{\hspace{1.5cm}}$ \\ [0.2em]
  ...
  \vspace{0.6em}
  5 units = 150 cm $\implies$ 1 unit = $\underline{\hspace{1.2cm}}$ cm \\ [0.2em]
...
% ================= EXAMPLE SECTION PART 4 =================
{\Large \textbf{Example: Working Backwards / Constant Difference}}
\vspace{0.3em}

\noindent Mark has \textbf{\$90} and Sam has \textbf{\$30}. How much money must Mark give to Sam so they both have the \textbf{same amount}?
\vspace{0.8em}

\begin{description}[leftmargin=2em]

  \item[\stepnum{1}{s41}] \textbf{Visualize the difference between the two quantities}\\[0.2em]
  Calculate how much extra money Mark has compared to Sam:
  \\[0.6em]
  \begin{tikzpicture}[>=Stealth, baseline={(0,0.5)}]
    \node[anchor=east] at (-0.2, 0.9) {\small Mark:};
    \draw[fill=bluebar, draw=black, line width=0.5pt] (0, 0.65) rectangle (4.5, 1.15);
    \node at (2.25, 0.9) {\small\color{white}\$90};

    \node[anchor=east] at (-0.2, 0.0) {\small Sam:};
    \draw[fill=greenbar, draw=black, line width=0.5pt] (0, -0.2) rectangle (1.5, 0.3);
    \node at (0.75, 0.05) {\small\color{white}\$30};
    ...
    \node[anchor=south] at (3.0, 1.35) {\small Difference: \$60};
  \end{tikzpicture}
  \vspace{0.6em}

  \item[\stepnum{2}{s42}] \textbf{Divide the difference into 2 equal parts}\\[0.4em]
  To make both sides equal, split the extra difference in half and give 1 part to Sam:
  \[
    \text{Difference} = 90 - 30 = 60
  \]
  \[
    \text{Amount to give} = 60 \div 2 = \mathbf{30}
  \]
  \vspace{0.2em}
  Mark must give Sam \textbf{\$30}.

\end{description}

\begin{tikzpicture}[remember picture, overlay]
  \draw[dotted, thick, textlight!60] (s41.south) -- (s42.north);
\end{tikzpicture}
...
{\Large \textbf{Practice Questions: Advanced Problems}}
\vspace{1em}

\begin{enumerate}[label=\textbf{\arabic*.}, start=15, itemsep=2.5em, leftmargin=2em]

  % Question 15
  \item \textbf{David had \$300. He spent $\frac{1}{3}$ of his money on books and $\frac{1}{4}$ of the remaining money on lunch. How much money did he have left?}

  \vspace{0.5em}
  \begin{tikzpicture}[>=Stealth, baseline={(0,0.5)}]
    \draw[fill=redbar, draw=black, line width=0.5pt] (0, 0.6) rectangle (1.8, 1.1);
    \draw[fill=bluebar, draw=black, line width=0.5pt] (1.8, 0.6) rectangle (5.4, 1.1);
    ...
    \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.5pt] (0, 1.2) -- (5.4, 1.2);
    \node[anchor=south] at (2.7, 1.3) {\small Total: \$300};

    \draw[decorate, decoration={brace, amplitude=4pt, mirror}, line width=0.5pt] (1.8, 0.5) -- (5.4, 0.5);
    \node[anchor=north] at (3.6, 0.4) {\small Remaining money divided into 4 equal parts};
  \end{tikzpicture}

  \vspace{0.6em}
  Money after books = \$300 - $\frac{1}{3}(300) =$ \underline{\hspace{1.5cm}} \\
  Lunch cost = $\frac{1}{4} \times \underline{\hspace{1.5cm}} =$ \underline{\hspace{1.5cm}} \\
  Money left = \underline{\hspace{1.5cm}} - \underline{\hspace{1.5cm}} = \$\underline{\hspace{1.5cm}}
  ...
  \begin{tikzpicture}[>=Stealth, baseline={(0,0.5)}]
    ...
    \draw[fill=bluebar, draw=black, line width=0.5pt] (0, 0.65) rectangle (4.0, 1.15);
    \node at (2.0, 0.9) {\small\color{white}2 $\times$ \$2.40 = \$4.80};

    \node[anchor=east] at (-0.2, 0.0) {\small Set 2 (2P + 3E):};
    \draw[fill=bluebar, draw=black, line width=0.5pt] (0, -0.2) rectangle (4.0, 0.3);
    \draw[fill=yellowbar, draw=black, line width=0.5pt] (4.0, -0.2) rectangle (4.75, 0.3);
    \node at (4.375, 0.05) {\small +1E};

    \draw[dashed, line width=0.5pt] (4.0, -0.2) -- (4.0, 1.15);
    \draw[decorate, decoration={brace, amplitude=4pt}, line width=0.5pt] (0, 1.25) -- (4.75, 1.25);
    \node[anchor=south] at (2.375, 1.35) {\small Total: \$5.70};
  \end{tikzpicture}

  \vspace{0.6em}
  Cost of 1 eraser = $\$5.70 - \$4.80 = \$\underline{\hspace{1.5cm}}$ \\
  Cost of 1 pencil = $\$2.40 - \$\underline{\hspace{1.2cm}} = \$\underline{\hspace{1.5cm}}$
